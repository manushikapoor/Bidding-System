...
import static com.manushi.product.util.Constants.ERROR_CODE_DATABASE_EXCEPTION;
import static com.manushi.product.util.Constants.ERROR_CODE_DATA_NOT_FOUND_EXCEPTION;
import static com.manushi.product.util.Constants.ERROR_CODE_DATETIME_PARSE_EXCEPTION;
import static com.manushi.product.util.Constants.ERROR_CODE_DUPLICATE_DATA_EXCEPTION;
import static com.manushi.product.util.Constants.ERROR_CODE_INTERNAL_SERVER_EXCEPTION;
import static com.manushi.product.util.Constants.ERROR_CODE_INVALID_REQUEST_EXCEPTION;
import static com.manushi.product.util.Constants.ERROR_MESSAGE_DATABASE_EXCEPTION;
import static com.manushi.product.util.Constants.ERROR_MESSAGE_DATETIME_PARSE_EXCEPTION;
import static com.manushi.product.util.Constants.LOG_PATTERN;
import static com.manushi.product.util.Constants.PRODUCT_SERVICE;
...
import com.manushi.product.repository.enums.ExceptionType;
...
public class CustomExceptionsHandler {
	...
	public final ResponseEntity<ErrorVO> handleInvalidRequestException(Exception ex) {
		...
		return ResponseEntity.badRequest()
				.body(buildErrorResponse(new InvalidRequestException(ex.getMessage()), ERROR_CODE_INVALID_REQUEST_EXCEPTION));
	}
	...
	public final ResponseEntity<ErrorVO> handleConstraintViolationException(ConstraintViolationException ex) {
		...
		if (!violations.isEmpty()) {
			...
			return ResponseEntity.badRequest()
					.body(buildErrorResponse(new InvalidRequestException(errorMessage), ERROR_CODE_INVALID_REQUEST_EXCEPTION));
		}
	...
	public final ResponseEntity<ErrorVO> handleUnauthorizedUserException(Exception ex) {
		log.info("AccessDeniedException occurred: [{}]", ex.getMessage());
	...
	public final ResponseEntity<ErrorVO> handleInvalidDateTimeException(Exception ex) {
		...
		return ResponseEntity.badRequest()
				.body(buildErrorResponse(new InvalidRequestException(ERROR_MESSAGE_DATETIME_PARSE_EXCEPTION), ERROR_CODE_DATETIME_PARSE_EXCEPTION));
	}
	...
	public final ResponseEntity<ErrorVO> handleDataAccessException(Exception ex) {
		log.error(LOG_PATTERN, PRODUCT_SERVICE, ExceptionType.DATABASE_ERROR, ERROR_CODE_DATABASE_EXCEPTION, ex);
		return ResponseEntity.internalServerError()
				.body(buildErrorResponse(new InvalidRequestException(ERROR_MESSAGE_DATABASE_EXCEPTION), ERROR_CODE_DATABASE_EXCEPTION));
	}

	@ExceptionHandler
	public final ResponseEntity<ErrorVO> handleException(Throwable ex, HttpServletRequest request) {
		log.error(LOG_PATTERN, PRODUCT_SERVICE, ExceptionType.INTERNAL_SERVER_ERROR, ERROR_CODE_INTERNAL_SERVER_EXCEPTION, ex);
		return ResponseEntity.internalServerError().body(buildErrorResponse(ex, ERROR_CODE_INTERNAL_SERVER_EXCEPTION));
	}
